...
# for(ssi in which(allspecies.eng %in% speciestemp2)){
  ...
  if(fy < 1990){
    pool_by_strat = rowMeans(pool[,-c(4:9)]) #summarizing the pooling for the outer knots
  }else{
    pool_by_strat = rowMeans(pool[,-c(1:9)]) #summarizing the pooling for the outer knots
  }
